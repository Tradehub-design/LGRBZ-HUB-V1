...
export function PortfolioHeatmap({ holdings }: { holdings: HeatmapHolding[] }) {
  if (!holdings.length) {
    return (
      <div className="rounded-xl border border-[#173047] bg-[#0b1e30] p-8 text-center text-sm text-slate-500">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      {holdings.slice(0, 12).map((holding) => {
        ...
        return (
          <div
            key={holding.id}
            className={`rounded-xl border p-4 shadow-xl transition hover:-translate-y-0.5 ${
              positive
