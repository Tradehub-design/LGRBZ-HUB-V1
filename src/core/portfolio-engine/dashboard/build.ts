...
function canonicalAllocationValue(
  holding: PortfolioHolding,
  ...
  const securityRecord =
    holding.security as unknown as
      Record<string, unknown>;

  const classificationRecord =
    holding.classification as unknown as
      Record<string, unknown>;

  const accountRecord =
    holding.account as unknown as
      Record<string, unknown>;
...
function normaliseAllocationIdentity(
  value: unknown,
): string {
  return String(
    value ??
...
function holdingMatchesAllocationSlice(
  holding: PortfolioHolding,
  dimension: AllocationDimension,
  slice: PortfolioAllocationSlice,
): boolean {
  const holdingValue =
    normaliseAllocationIdentity(
      canonicalAllocationValue(
        holding,
        dimension,
      ),
    );
  ...
  if (
    ...
    return sliceCandidates.some(
      (candidate) =>
        securityCandidates.includes(
          candidate,
        ),
    );
  }

  return sliceCandidates.includes(
    holdingValue,
  );
}
